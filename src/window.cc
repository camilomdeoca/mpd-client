...
bool Window::button_press (QMouseEvent * event)
{
    ...
    dock_move_start (m_id, event->globalPosition ().x (), event->globalPosition ().y ());
    m_is_moving = true;
    return true;
}
...
bool Window::motion (QMouseEvent * event)
{
    ...
    dock_move (event->globalPosition ().x (), event->globalPosition ().y ());
    return true;
}
...
{
    return config.active_titlebar_any ? false : isActiveWindow ();
}
