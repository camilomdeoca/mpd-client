...
void SkinnedVis::set_colors ()
{
    ...
    for (int x = 0; x < 256; x ++)
    {
        unsigned char r = std::min (x, 127) * 2;
        unsigned char g = std::clamp (x - 64, 0, 127) * 2;
        unsigned char b = std::max (x - 128, 0) * 2;
        m_voice_color_fire[x] = qRgb (r, g, b);
    }

    for (int x = 0; x < 256; x ++)
        m_voice_color_ice[x] = qRgb (x / 2, x, std::min (x * 2, 255));
...
void SkinnedVis::draw (QPainter & cr)
{
    ...
    case VIS_SCOPE:
        ...
        case SCOPE_DOT:
            for (int x = 0; x < 75; x ++)
            {
                int h = std::clamp ((int) m_data[x], 0, 15);
                RGB_SEEK (x, h);
        ...
        case SCOPE_LINE:
        {
            ...
            {
                int h = std::clamp ((int) m_data[x], 0, 15);
                int h2 = std::clamp ((int) m_data[x + 1], 0, 15);
            ...
            int h = std::clamp ((int) m_data[74], 0, 15);
            RGB_SEEK (74, h);
            RGB_SET_INDEX (vis_scope_colors[h]);
            break;
        }
