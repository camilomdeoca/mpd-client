...
static void eq_on_cb (Button * button, QMouseEvent * event)
{
    ...
    settings.equalizer_active = button->get_active();
}
...
bool EqWindow::button_press (QMouseEvent * event)
{
    if (event->button () == Qt::LeftButton &&
     ...
     event->position ().y () < 14 * config.scale)
    {
        /// TODO: Activate this when shading works
...
static void eqwin_balance_motion_cb ()
{
    eqwin_balance_set_knob ();
    int pos = equalizerwin_balance->get_pos ();
    pos = std::min(pos, 38);    /* The skin uses a even number of pixels
...
static void equalizerwin_create_widgets ()
{
    ...
    equalizerwin_on->set_active (true);
    ...
    equalizerwin_preamp = new EqSlider ("Preamp", -1);
    equalizerwin->put_widget (false, equalizerwin_preamp, 21, 38);
    //equalizerwin_preamp->set_value (aud_get_double ("equalizer_preamp"));

    const std::string bandnames[AUD_EQ_NBANDS] = {"31 Hz",
    ...
    for (int i = 0; i < AUD_EQ_NBANDS; i ++)
    {
        equalizerwin_bands[i] = new EqSlider (bandnames[i], i);
...
{
    bool shaded = settings.skins.equalizer_shaded;
    ...
    equalizerwin->setWindowTitle ("Audacious Equalizer");
...
void equalizerwin_create ()
{
    ...
    equalizerwin_preamp->set_value(equalizer_preamp);
