...
void TextBox::render_vector (const std::string & text)
{
    QRect ink = m_metrics->tightBoundingRect (text.c_str());

#if QT_VERSION >= QT_VERSION_CHECK(5, 11, 0)
    int logical_width = m_metrics->horizontalAdvance (text.c_str());
#else
    ...
    int width = std::max (-ink.x () + logical_width, 1);
    int height = std::max (ink.height (), 1);

    resize (m_width * config.scale, height);

    m_buf_width = std::max ((width + config.scale - 1) / config.scale, m_width);
    m_buf.reset (new QImage (m_buf_width * config.scale, height, QImage::Format_RGB32));
    ...
    cr.drawText (-ink.x (), -ink.y (), text.c_str());
}
...
void TextBox::render_bitmap (const std::string & text)
{
    ...
    QList<uint> ucs4 = QString (text.c_str()).toUcs4 ();

    m_buf_width = std::max (cw * (int) ucs4.length (), m_width);
    m_buf.reset (new QImage (m_buf_width * config.scale, ch * config.scale,
     QImage::Format_RGB32));
...
void TextBox::render ()
{
    ...
    const std::string text = m_text.c_str();
    ...
    if (m_may_scroll && m_buf_width > m_width)
    {
        ...
        if (! m_two_way)
        {
            ...
            if (m_font)
                render_vector (temp.c_str());
            else
                render_bitmap (temp.c_str());
        }
    }
    ...
    if (m_scrolling) {
        //scroll_timer.start ();
        //connect(m_timer, &QTimer::timeout, this, QOverload<>::of(&TextBox::scroll_timeout));
        m_timer->start(1000/30);
    }
    else {
        //scroll_timer.stop ();
        //disconnect(m_timer, &QTimer::timeout, this, QOverload<>::of(&TextBox::scroll_timeout));
        m_timer->stop();
    }
}
...
void TextBox::set_font (const std::string & font)
{
    if (!font.empty())
    {
...
TextBox::~TextBox ()
{
    std::vector<TextBox *>::iterator iter = textboxes.begin();
    while (iter != textboxes.end()) {
        if (*iter == this) {
            textboxes.erase(iter);
            break;
        }
        iter++;
    }
...
TextBox::TextBox (int width, const std::string & font, bool scroll) :
    m_width (width),
    m_may_scroll (scroll),
    m_two_way (config.twoway_scroll)
{
