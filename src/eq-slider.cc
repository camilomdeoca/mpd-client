...
void EqSlider::moved (int pos)
{
    ...
    if (m_band < 0)
        /// TODO: Here the preamp would be set
        equalizer_preamp = m_value;
    else
        /// TODO: Here the equalizer band value would be set
        bands[m_band] = m_value;

    mainwin_show_status_message (str_printf ("%s: %+.1f dB", m_name.c_str(), m_value));
}
...
bool EqSlider::button_press (QMouseEvent * event)
{
    ...
    m_pressed = true;
    moved (event->position ().y () / config.scale - 5);
    queue_draw ();
    return true;
}
...
void EqSlider::set_value (float value)
{
    ...
    m_value = value;
    m_pos = std::clamp (25 - (int) (value * 25 / AUD_EQ_MAX_GAIN), 0, 50);
    queue_draw ();
}

EqSlider::EqSlider (const std::string & name, int band) :
    m_name (name),
    m_band (band)
{
