...
{
    unsigned int selected_songs_duration_ms = 0;
    for (int i = 0; i < mpd.status().get_queue_length(); i++)
        if (playlistwin_list->is_selected(i))
            selected_songs_duration_ms += mpd.queue()[i].get_duration_ms();
    std::string s1 = str_format_time (selected_songs_duration_ms);
    std::string s2 = str_format_time (mpd.queue ().get_total_duration_ms ());
    playlistwin_info->set_text (str_printf ("%s/%s", s1.c_str(), s2.c_str()));
}
...
static void update_rollup_text ()
{
    ...
    {
        std::string title = mpd.current_song().get_title();
        int length = mpd.current_song().get_duration_ms();

        if (settings.show_numbers_in_pl)
            APPEND (scratch, "%d. ", 1 + mpd.status().get_current_song_index_in_queue());

        APPEND (scratch, "%s", title.c_str ());

        if (length >= 0)
        {
            std::string buf = str_format_time (length);
            APPEND (scratch, " (%s)", buf.c_str ());
        }
    }
...
//{
//    view_set_playlist_shaded (! settings.skins.playlist_shaded);
//}
...
static void playlistwin_resize (int w, int h)
{
    ...
    if (! settings.skins.playlist_shaded)
    {
...
bool PlWindow::button_press (QMouseEvent * event)
{
    if (event->button () == Qt::LeftButton &&
     event->type () == QEvent::MouseButtonDblClick && event->position ().y () < 14)
    {
    ...
    {
        //menu_popup (UI_MENU_PLAYLIST, event->globalPosition ().x (), event->globalPosition ().y (), false, false);
        return true;
    }
...
{
    playlistwin_time_min->set_text ("");
    playlistwin_time_sec->set_text ("");
}

void playlistwin_set_time (const std::string & minutes, const std::string & seconds)
{
...
{
    bool shaded = settings.skins.playlist_shaded;
...
static void playlistwin_create_widgets ()
{
    ...
    bool shaded = settings.skins.playlist_shaded;
    playlistwin_sinfo = new TextBox (w - 35, "", shaded && config.autoscroll);
    ...
    std::string font = settings.skins.playlist_font;
...
static void playlistwin_create_window ()
{
    ...
    playlistwin->setWindowTitle ("Audacious Playlist Editor");
...
static void follow_cb ()
{
    playlistwin_list->select_all(false);

    int row = mpd.status ().get_current_song_index_in_queue ();
    if (row >= 0)
    {
        playlistwin_list->select (row, true);
...
void playlistwin_create ()
{
    ...
    mpd.add_callback(MPD_EVENT_QUEUE_UPDATE, update_cb);
    playlistwin_list->set_selection_change_callback(update_info); // TODO: Make sure this is all that has to change
    //mpd.add_callback(MPD_EVENT_QUEUE_POSITION, update_cb);
    mpd.add_callback(MPD_EVENT_QUEUE_PLAYED_SONG, follow_cb);
